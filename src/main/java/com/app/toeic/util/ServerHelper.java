...
public class ServerHelper {
    ...
    public String getCurrentUserIp() {
        ...
        return requestAttributes.map(servletRequestAttributes -> DEFAULT_HEADER.stream().map(header -> {
            var ipList = servletRequestAttributes.getRequest().getHeader(header);
            if (StringUtils.isNoneBlank(ipList) && !"unknown".equalsIgnoreCase(ipList)) {
                return Arrays.stream(ipList.split(",")).findFirst().orElse(defaultIp);
            }
            return defaultIp;
        }).findFirst().orElse(defaultIp)).orElse(defaultIp);
